...
class SetBudgetFragment : Fragment() {
    ...
    private fun saveBudgetToFirebase(onComplete: () -> Unit) {
        val userId = FirebaseAuth.getInstance().currentUser?.uid ?: return
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        AlertDialog.Builder(requireContext())
                            .setTitle("Budget Exists")
                            .setMessage("A budget for this category already exists. Do you want to update it?")
                            .setPositiveButton("Yes") { _, _ ->
                                val existingKey = snapshot.children.first().key ?: return@setPositiveButton
                                ...
                                dbRef.child(existingKey).setValue(updatedData)
                            ...
                            .setNegativeButton("No") { _, _ -> onComplete() }
                            .show()
                    } else {
                        val newBudgetId = dbRef.push().key ?: return
                        ...
                        dbRef.child(newBudgetId).setValue(budgetData)
                            .addOnSuccessListener {
    ...
    override fun onStart() {
        ...
        if (!hasSeenCalcTut) {
            val imageRes = R.drawable.anti
            try {
                val tutorialOverlay = TutorialFragment.newInstance(
                    imageRes,
                    "I remember my mother telling me 'There is McDonald's at the Colony' – good times!\n\n" +
                            "You can set your budgets here.\n" +
                            "• Select a category\n" +
                            "• Set an amount\n" +
                            "• Define the exact value using the slider, then click 'Define'\n\n" +
                            "Tap OK to begin!"
                )
                ...
                Toast.makeText(context, "Tutorial image not found", Toast.LENGTH_SHORT).show()
            }
        }
    }
}
