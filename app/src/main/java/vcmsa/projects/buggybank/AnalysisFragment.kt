...
class AnalysisFragment : Fragment() {
    ...
    private fun analyzeData() {
        ...
        transactionsRef.get().addOnSuccessListener { snapshot ->
                ...
                when (type) {
                    "Expense" -> {
                        ...
                        expensesByCategory[cat] =
                            expensesByCategory.getOrDefault(cat, 0.0) + amount
            ...
            val highestExpenseCategory =
                expensesByCategory.maxByOrNull { it.value }?.key ?: "N/A"
            
            // Display results in TextViews
            view?.findViewById<TextView>(R.id.txtTotalExpensesData)?.text =
                "R %.2f".format(totalExpenses)
            view?.findViewById<TextView>(R.id.txtTotalIncomeData)?.text =
                "R %.2f".format(totalIncome)
            view?.findViewById<TextView>(R.id.txtHighestExpenseData)?.text =
